...
TEST(GraphTransformationTests, IdentityElimination) {
  ...
  auto rule_transformer_L1 =
    std::make_unique<RuleBasedGraphTransformer>("RuleTransformerL1", "Level1 rule transformer");
...
TEST(GraphTransformationTests, ConstantFolding1) {
  ...
  rule_transformer_L1->Register(std::make_unique<ConstantFolding>());
  onnxruntime::GraphTransformerManager graph_transformation_mgr{5};
